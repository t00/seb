...
/*
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(closeDocument:)
                                               ...
                                               object:self];
                                               //accept only notification from myself
*/	
/*#ifdef DEBUG
...
- (void)webView:(WebView *)sender didReceiveTitle:(NSString *)title forFrame:(WebFrame *)frame
{
    ...
    if (frame == [sender mainFrame]){
        [self setTitle:[@"Safe Exam Browser:  " stringByAppendingString:title]];
    }
}
...
decisionListener:(id <WebPolicyDecisionListener>)listener {
     ...
     currentMainHost = [[MyGlobals sharedMyGlobals] currentMainHost];
     requestedHost = [[request mainDocumentURL] host];
     ...
     if (currentMainHost && requestedHost) {
         if (![currentMainHost isEqualToString:requestedHost]) {
             [listener ignore];
             // Get document for my web view
             id myDocument = [[NSDocumentController sharedDocumentController] documentForWindow:[webView window]];
             // Close document and therefore also window
             ...
             [[NSNotificationCenter defaultCenter]
              postNotificationName:@"switchPluginsOn" object:self];
             /*// Send notification to savely close this document/window
             [[NSNotificationCenter defaultCenter]
              postNotificationName:@"requestDocumentClose" object:self]; */
         }
     }
     [listener use];
...
- (void)webViewClose:(WebView*)sender
{
    // Get document for my web view
    id myDocument = [[NSDocumentController sharedDocumentController] documentForWindow:[sender window]];
    // Close my document and therefore also window
    [myDocument close];
}
*/

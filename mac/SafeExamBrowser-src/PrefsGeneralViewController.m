...
- (IBAction) savePrefsToAppBundle:(id)sender {
    ...
    if (![prefsDict writeToFile:prefsPath atomically:YES]) {
        ...
        NSRunAlertPanel(NSLocalizedString(@"Writing Settings to App Bundle Succeeded", nil), NSLocalizedString(@"Now you can deploy this copy of the Safe Exam Browser application to other machines. The settings you have just written to the app bundle will be used after every start of this pre-configured copy of SEB. If you want to change those initial settings, you have to either write the changed settings again to the app bundle or use the \"Remove from App Bundle\" button.", nil), NSLocalizedString(@"Ok", nil), nil, nil);               
    }
}
...
- (IBAction) removePrefsFromAppBundle:(id)sender {
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:fullPath]) {
                    ...
                    if (itemRemoved == NO) {
                        ...
                        NSString *errorText = [NSLocalizedString(@"Probably you don't have write permissions. Try to first move or copy the app to a directory you have permission to write in (like Desktop) or change the file permissions manually in Finder.\n", nil) stringByAppendingString:[error localizedDescription]];

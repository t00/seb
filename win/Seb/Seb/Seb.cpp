                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_WINKEYOX, szWindowClass, MAX_LOADSTRING);
	...
	*/
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_WINKEYOX);
	...
	*/
	/* main message loop */
	/*
	If the autostart process is finished it signals and closes WinKeyox. 
	...
	In this case "SHUTDOWN_AFTER_PROCESS_TERMINATES" should be empty (!) and the only way to finish WinKeyox 
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_WINKEYOX);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= (LPCTSTR)IDC_WINKEYOX;
	wcex.lpszClassName	= szWindowClass;
	wcex.hIconSm		= LoadIcon(wcex.hInstance, (LPCTSTR)IDI_WINKEYOX);
	...
	*/
	...
	if (!IsNewOS) {
		...
		if (getBool("NEW_DESKTOP")) {
			...
			hNewDesktop = CreateDesktop(WINKEYOX_DESK, NULL, NULL, 0, GENERIC_ALL, NULL);
	...
	AppendMenu(hSubMenu,MF_STRING,32773,"6789");
	AppendMenu(hMenu,MF_STRING,(UINT)hSubMenu,"123456");
	*/
	//DrawMenuBar(hWnd);
	//IDM_STARTPROCESS
...
BOOL ReadIniFile()
{
	...
	try
	{
		...
		if (sProcesses != "") {
			...
			//mpProcessCommands.insert(make_pair(cntCommand,vProcess));
			/* Make PROCESS_COMMANDS */			
		}
...
BOOL EditRegistry()
{
	...
	{	
		...
		{
			if (!HandleSetRegistryKeyValue(hkSystem,VAL_DisableLockWorkstation,"DISABLE_LOCK_WORKSTATION")) return FALSE;
		}
		if (getBool("REG_DISABLE_TASKMGR")) 
		{ 			
			if (!HandleSetRegistryKeyValue(hkSystem,VAL_DisableTaskMgr,"DISABLE_TASKMGR")) return FALSE;
		} 
		...
		{ 	
			if (!HandleSetRegistryKeyValue(hkExplorer,VAL_NoClose,"NO_CLOSE")) return FALSE;
		} 
		if (getBool("REG_NO_LOGOFF"))
		{ 			
			if (!HandleSetRegistryKeyValue(hkExplorer,VAL_NoLogoff,"NO_LOGOFF")) return FALSE;
		}
	}
	catch( char * str )
	{		
...
BOOL ResetRegistry()
{
	...
	{	
		...
		if (getBool("REG_DISABLE_TASKMGR")) 
		{ 		
			RegDeleteValue(hkSystem,VAL_DisableTaskMgr);
		} 
		if (getBool("REG_DISABLE_CHANGE_PASSWORD")) 
		{ 			
			RegDeleteValue(hkSystem,VAL_DisableChangePassword);
		}
		if (getBool("REG_NO_CLOSE"))
		{ 				
			RegDeleteValue(hkExplorer,VAL_NoClose);
		} 
		if (getBool("REG_NO_LOGOFF"))
		{ 			
			RegDeleteValue(hkExplorer,VAL_NoLogoff);
		}
...
BOOL CreateExternalProcess(string sProcess)
{
	...
	*/
	...
	if (IsNewOS && getBool("NEW_DESKTOP")) {
		siProcess.lpDesktop = WINKEYOX_DESK;
	}
	...
	pSec.bInheritHandle = sizeof(pSec);
...
BOOL ShutdownInstance()
{
	...
	for( itProcessInformations = mpProcessInformations.begin(); itProcessInformations != mpProcessInformations.end(); itProcessInformations++ ) {
		...
		of << mpProcesses[(*itProcessInformations).first].c_str();
		of << "\n";
		of.close();	
		*/
		/* Look at STRONG_KILL_PROCESSES */
		//..... 
		/*
		sCommandLine = mpProcesses[(*itProcessInformations).first];
		Tokenize(sCommandLine,vCommandLine," ");
		sExecutable = vCommandLine[0];
		Tokenize(sExecutable,vExecutable,"/");
		if (vExecutable.size() > 0) {			
			ofstream of("log.txt",ios::app);
			of << vExecutable[0].c_str();
			of << "\n";
			of.close();	
		}
		*/
		//if (getBool("STRONG_KILL_PROCESSES")) {
		
		/*
		sCommandLine = mpProcesses
		*/
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
        ...
        SetMenu(hWnd, hMenu);
		/*	
		hIcon = LoadImage(NULL, "WinKeyox.ico", IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
		if(hIcon)
			SendMessage(hWnd, WM_SETICON, ICON_BIG, (LPARAM)hIcon);
		else
			MessageBox(hWnd, "Could not load large icon!", "Error", MB_OK | MB_ICONERROR);

		hIconSm = LoadImage(NULL, "WinKeyox.ico", IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
		if(hIconSm)

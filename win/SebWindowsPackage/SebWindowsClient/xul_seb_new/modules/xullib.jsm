...
var xullib = (function () {	
			...
			if (defaultProfile.exists()) {
				...
				while(entries.hasMoreElements()) {  
					...
					if (!cf.exists()) {
						...
						_debug(entry.leafName + " already exists in " + profile.dirs[0].path);
					}					
				}
	...
	function addWin(obj) {
		...
		initBrowser(w);
		wins.push(w);
		_debug("window added with type: " + getWinType(w));
	}
	
	function cleanWins() {
		_debug("cleanWins");
		for (var i=0;i<wins.length;i++) {
			if (!wins[i].document) { // no document object -> remove
				removeWin(wins[i]);
			}
		}
	}
	function removeWin(win) {
		for (var i=0;i<wins.length;i++) {
			if (wins[i] === win) {
				var n = (wins[i].document) ? getWinType(win) + ": " + win.document.title : " empty document";
				//_debug("remove win from array: " + ;
				_debug("remove win from array: " + n);
				wins.splice(i,1);
				break;
			}
		}
	...
	function loadPage(win,uri,loadFlag) {	
		_debug("try to load: " + uri);			
		...
		win.XulLibBrowser.webNavigation.loadURI(uri, wnav.LOAD_FLAGS_NONE, null, null, null);		
	}
